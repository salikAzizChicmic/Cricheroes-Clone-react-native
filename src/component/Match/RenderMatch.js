  ...
  return (
    ...
    <TouchableOpacity onPress={()=>navigation.navigate("Dashboard")} style={{width:'100%',height:60,backgroundColor:'white'}}>
        ...
        <ScrollView showsHorizontalScrollIndicator={false} showsVerticalScrollIndicator={false}>
                    {alldata.map((ele,ind)=>
                        {
                            return <ViewMatchList key={ind} name={ele.name} city={ele.city} date={ele.date} time={ele.time} ground={ele.ground} over={ele.over} perbowler={ele.perbowler} status={ele.status} />
                        }
                    )}
        </ScrollView>
    </View>
